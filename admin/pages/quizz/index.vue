	...
	section.container-fluid
		header.row
			div.col-12.py-3.d-flex.flex-row.flex-wrap.align-items-center
				h2.pb-0.mb-0 Tout les quizz
				nuxt-link.btn.btn-success.btn-sm.mx-3(:to="localePath({name: 'quizz-add'})") Ajouter un nouveau quizz
		div.row.p-3
			div.col-12.mb-3.d-flex.flex-row.flex-wrap.px-3.card.py-3
				div.col.statusButton.px-0(
					...
					nuxt-link.btn.btn-transparent.publish(
						:to="localePath({name:'quizz', hash: '#publish'})"
					...
					nuxt-link.btn.btn-transparent.draft(
						:to="localePath({name:'quizz', hash: '#draft'})"
					...
					nuxt-link.btn.btn-transparent.archived(
						:to="localePath({name:'quizz', hash: '#archived'})"
			...
			div.w-100.position-relative.mb-3
				vuetable.w-100.card.rounded-8px.mb-0(
					...
					div.w-100(slot="title" slot-scope="props")
						nuxt-link(:to="localePath({name : 'quizz-id', params: {id : props.rowData.id}})") {{props.rowData.title}}
...
import FieldsDef from "~/assets/data/quizzHandle.js";
export default {
	components: {
		Vuetable,
		VuetablePagination
	...
	async fetch() {
		this.loading = true
		this.data = await fetch("http://127.0.0.1:8000/api/quizz").then(res => {
			return res.json()
		}).then(r => {
