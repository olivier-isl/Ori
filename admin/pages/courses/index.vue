	...
	section.container-fluid
		header.row
			div.col-12.py-3.d-flex.flex-row.flex-wrap.align-items-center
				h2.pb-0.mb-0 Tout les cours
				nuxt-link.btn.btn-success.btn-sm.mx-3(:to="localePath({name: 'courses-add'})") Ajouter un nouveau cours
		div.row.p-3
			div.col-12.mb-3.d-flex.flex-row.flex-wrap.px-3.card.py-3
				div.col.statusButton.px-0(
					...
					nuxt-link.btn.btn-transparent.all(
						:to="localePath({name:'courses', hash: ''})"
					...
					nuxt-link.btn.btn-transparent.publish(
						:to="localePath({name:'courses', hash: '#publish'})"
					...
					nuxt-link.btn.btn-transparent.draft(
						:to="localePath({name:'courses', hash: '#draft'})"
					...
					nuxt-link.btn.btn-transparent.archived(
						:to="localePath({name:'courses', hash: '#archived'})"
			...
			div.w-100.position-relative.mb-3
				vuetable.w-100.card.rounded-8px.mb-0(
					...
					div.w-100(slot="title" slot-scope="props")
						nuxt-link(:to="localePath({name : 'courses-id', params: {id : props.rowData.id}})") {{props.rowData.title}}
					...
					div.w-100.text-right(slot="actions" slot-scope="props")
						nuxt-link.btn.btn-light.mx-1(
							:to="localePath({name : 'courses-id', params: {id : props.rowData.id}})"
...
import FieldsDef from "~/assets/data/coursesHandle.js";
export default {
	components: {
		Vuetable,
		VuetablePagination
	...
	async fetch() {
		this.loading = true
		this.data = await fetch("http://127.0.0.1:8000/api/courses").then(res => {
			return res.json()
		}).then(r => {
	...
	methods: {
			...
			if(data.status !== 'archived') {
				this.$swal({
					...
					if(res.isConfirmed) {
						...
						this.$axios.$put("http://127.0.0.1:8000/api/courses/" + data.id, {
							status : "archived"
						}).then(res => {
							this.$fetch()
							this.$swal.fire({
				...
				this.$swal({
					...
					if(res.isConfirmed) {
						this.$axios.$delete("http://127.0.0.1:8000/api/courses/" + data.id).then(res => {
							console.log(res)
							this.$fetch().then(() => {
		...
		getByStatus(status) {
			if(this.data_origin.length <= 0) {
				this.$axios.$get("http://127.0.0.1:8000/api/statuscourses/"+status).then(res => {
					this.data = res.data
					this.loading = false
				})
			} else {
