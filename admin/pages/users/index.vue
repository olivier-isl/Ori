	...
	section.container-fluid
		header.row
			div.col-12.py-3.d-flex.flex-row.flex-wrap.align-items-center
				h2.pb-0.mb-0 Tout les utilisateurs
				nuxt-link.btn.btn-outline-primary.btn-sm.mx-3(:to="localePath({name: 'users-add'})") Ajouter un nouvel utilisateur
		div.row.p-3
			div.col-12.mb-3.d-flex.flex-row.flex-wrap.justify-content-end.px-3.card.py-3
				div.col-2.px-0
					input.form-control(
						type="text"
						v-model="search"
			...
			div.w-100.position-relative.mb-3
				vuetable.w-100.card.rounded-8px.mb-0(
					...
					div.w-100(slot="login" slot-scope="props")
						nuxt-link(:to="localePath({name : 'quizz-id', params: {id : props.rowData.id}})") {{props.rowData.login}}
					div.w-100(slot="created_at" slot-scope="props")
						span {{getDate(props.rowData.created_at)}}
					div.w-100.text-right(slot="actions" slot-scope="props")
						nuxt-link.btn.btn-light.mx-1(
							:to="localePath({name : 'quizz-id', params: {id : props.rowData.id}})"
...
import FieldsDef from "~/assets/data/membersHandle.js";
export default {
	components: {
		Vuetable,
		VuetablePagination
	...
	async fetch() {
		this.loading = true
		this.data = await fetch("http://127.0.0.1:8000/api/users").then(res => {
			return res.json()
		}).then(r => {
	...
	methods: {
			...
			if(data.status !== 'archived') {
				this.$swal({
					...
					if(res.isConfirmed) {
						...
						this.$axios.$put("http://127.0.0.1:8000/api/quizz/" + data.id, {
							status : "archived"
						}).then(res => {
							this.$fetch()
							this.$swal.fire({
				...
				this.$swal({
					...
					html : 'Souhaitez-vous réellement supprimer ce quizz ?<br><br><b>Attention, vous ne pourrez-plus récupérer ce contenu.</b><br><br>En êtes-vous réellement sûr ?',
					showConfirmButton : true,
					showCancelButton : true,
					...
					if(res.isConfirmed) {
						this.$axios.$delete("http://127.0.0.1:8000/api/quizz/" + data.id).then(res => {
							console.log(res)
							this.$fetch().then(() => {
		...
		getAll() {
			if(this.data_origin.length <= 0) {
				this.$axios.$get("http://127.0.0.1:8000/api/quizz").then(res => {
		...
		getByStatus(status) {
			if(this.data_origin.length <= 0) {
				this.$axios.$get("http://127.0.0.1:8000/api/statusquizz/"+status).then(res => {
					this.data = res.data
					this.loading = false
				})
			} else {
